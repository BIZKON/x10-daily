...
- **Каждый клиент — отдельная копия системы** (свой бот, канал, домен, база, кошелёк AI Gateway). `tenant_id` НЕ вводится: клиента отличает конфигурация экземпляра. Общей базы с разделением по тенантам не будет — это переписало бы каждый существующий запрос и экран.
