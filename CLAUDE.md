...
### 🔴 Главное с 06.08.2026: это контент-завод на продажу

ProAgent AI — не одно медиа, а **машина автогенерации контента, которую продают клиентам с настройками под них**. Собственный канал «ИИ работает на вас» — витрина и первый жилец завода, а не его предназначение.

**Критерий готовности ЛЮБОЙ функции:** смогу ли я подключить это клиенту быстро — настройками, без правок кода и сложных доделок? Работает через хардкод, ручной SQL или правку env на VM — значит **не готово**, даже если работает у нас.

Следствия, обязательные к учёту при проектировании:

- **Каждый клиент — отдельная копия системы** (свой бот, канал, домен, база). `tenant_id` НЕ вводится: клиента отличает конфигурация экземпляра. Общей базы с разделением по тенантам не будет — это переписало бы каждый существующий запрос и экран.
- **Токены оплачиваем МЫ, клиент платит нам ×3** (решение 06.08.2026, вечер). Ключ AI Gateway в копии клиента — наш. ⚠️ Это отменило более раннее решение того же дня про «свой кошелёк у каждого клиента»: при своём кошельке клиент видел бы счёт от Timeweb напрямую, и наценка расходилась бы с реальностью. Себестоимость и цена считаются в рублях; рублёвая сумма фиксируется в момент прогона (`pipeline_runs.cost_rub`), иначе счёт менялся бы задним числом при правке курса.
- **Команда клиента работает сама:** роли Владелец · Редактор · Автор · Наблюдатель, приглашение одноразовой ссылкой. Роль ≠ «Роль» в карточке автора (та — подпись под статьёй). См. Спеку 5.
- **Админка обязана объяснять себя без нас.** У каждого поля — раскрывающееся «Зачем это поле» по-русски: что это, зачем, что будет если пусто. Клиент не читает наши доки и не может спросить разработчика. Образец — формы авторов и событий.
- **Бренд, голос, рубрикатор, чёрный список, слоты, источники парсинга — конфигурация клиента**, а не константы репозитория.
...
| **Состояние на конец последней сессии** | **`docs/handoffs/handoff-session-36.md`** |
