 ...
 *
 * 🔴 Константа общая с админкой (`@x10/config`): по этой же границе экран
 * «Постинг» отличает строки, которые ждут слота, от тех, которые уже не выйдут
 * никогда. Разойдись они — экран обещал бы публикации, которых не будет.
 */
const STALE_HOURS = POST_SLOT_STALE_HOURS;
