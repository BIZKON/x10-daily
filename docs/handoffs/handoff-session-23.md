...
- **⚠️→✅ UPDATE (session 23, итог): DeepSeek сначала откатывали (deepseek-chat function-calling валил ToV 0/10), затем СДЕЛАЛИ response_format-фикс → DeepSeek АКТИВЕН, Claude OFF.** `define-agent` стал provider-conditional: deepseek → `response_format json_object` + JSON Schema в промпт + парс `message.content`; Claude-путь без изменений. + FactCheck enum-resilience (`.catch`) + `zodToToolSchema` nullable→null. Валидация на deepseek-chat: main chain 7/7, ToV 10/10, FactCheck корректен. `MODEL_*=deepseek/deepseek-chat` в `.env.production`, pipeline healthy, live `modelUsed=deepseek/deepseek-chat` → **ZERO Claude**. HEAD `fe52439`. Рабочий id — `deepseek/deepseek-chat` (= V4 Flash non-thinking; thinking-варианты `v4-flash`/`v4-pro` → HTTP 400 на forced tool_choice; `json_schema` strict на gateway недоступен). Watch-item: лёгкая модель → FactCheck слабее Claude Opus. Детали — memory `project_x10_deploy_state.md`. (§4 ниже — историческая попытка-1, устарела.)
