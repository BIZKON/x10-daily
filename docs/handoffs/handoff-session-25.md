...
**HEAD кода:** `7e77f01` · `origin/main` synced · задеплоено (`./deploy.sh`, 1 деплой).
...
- **Остаток miniapp:** картинки вместо unsplash (VisualAgent post-M0); auth UX (401-recovery); ⚠️ лента при API-down в рантайме всё ещё показывает мок-FEED со слагами-404 (редкий edge — сделать link-safe, как у hero).
...
>
> Состояние: M0 + walking-skeleton ЖИВ+АВТОНОМЕН на Timeweb. **HEAD кода `7e77f01`.** Автономный постинг 4/день (DeepSeek v4-flash, IPv6-watchdog). **Miniapp ЖИВ+наполнен** (app.pro-agent-ai.ru): лента 40+, читалка, **real digest-hero** (синтез из топ-статей, без выдуманных цитат). Mini App в BotFather НЕ запущен (handoff-s24 §4).
>
> Session 25: real digest-hero — новый `GET /v1/digests/hero` (editorial-first→синтез из топ-статей, `/latest` не тронут), home переведён на **PPR-дыры** (`connection()` внутри Suspense — иначе build запекал мок-fallback со слагами-404). Adversarial-ревью (10 агентов, 4 находки) + live-верифицировано.
>
