 ...
 * ID каналов — внутренние (legacy): tg-rybakov = авторский канал основателя,
 * tg-x10 = канал «ИИ работает на вас!». Прошиты в pipeline и articles.metadata.
 */
...
const CHANNEL_RULES: Record<HookChannel, string> = {
  "tg-rybakov": [
    "Авторский канал основателя ProAgent AI. Первое лицо («Я», «У нас»). Прямой, без регалий, без «коллеги»",
    ...
    "CTA: вопрос-зацепка или приглашение обсудить внедрение ИИ-агентов",
  ].join("\n"),
  "tg-x10": [
    "Канал «ИИ работает на вас!» — официальный канал ProAgent AI. Деловой, сухой, от редакции. Без «я»",
    "200-300 слов, абзацы по 1-2 предложения, разделены пустой строкой",
    "Без эмодзи. Цифры с источником в скобках",
    "Выгода для бизнеса — в первых двух абзацах: часы, деньги или конверсия",
    "Только русский: без англицизмов где есть русский эквивалент, без латинских заголовков-меток в тексте",
    "CTA: «читать полностью в ProAgent AI», без манипуляций",
  ].join("\n"),
  vk: [
...
function buildSystem(): string {
  return `Ты — SocialAmplifyAgent редакции ProAgent AI. Получаешь сжатую статью и упаковываешь её в пост под конкретный канал и framework.

VOICE RULES (общие для всех каналов ProAgent AI):
${VOICE_RULES}
...
2. Применить framework как НЕВИДИМУЮ структуру: мысленно разложи контент на stages, но имена стадий в тексте НЕ печатай
3. Применить per-channel rules (см. ниже) — длина, формат, стиль обращения
4. Написать post — готовый к публикации текст ЧИСТОЙ РУССКОЙ ПРОЗОЙ, с настоящими переносами строк. Структуру задавай абзацами и связками, а НЕ заголовками-метками
5. Заполнить segments — разбивка по стадиям с stage-именем (английское имя стадии — ТОЛЬКО здесь, для админки; в поле post его быть НЕ должно)
...
- post — ТОЛЬКО на русском. НИ ОДНОГО англоязычного заголовка-метки в начале абзаца: ни имён стадий framework (Before / After / Bridge / Problem / Agitation / Solution и т.п.), ни англо-меток Smart Brevity (Yes, but / What's next / Why it matters / Bottom line). Это касается ТОЛЬКО поля post; в segments английские имена стадий допустимы
- Контраст и продолжение — по-русски: «Но…», «Да, но…» вместо «Yes, but»; «Что дальше:», «Следим:» вместо «What's next»
- Без англицизмов где есть русский эквивалент (sandbox → песочница, hackathon → хакатон). Тикеры (IMOEX) допустимы, но «Мосбиржа» предпочтительнее «MOEX»

ВОЗВРАЩАЙ через tool_use x10_emit_social. Поле post — финальный текст с НАСТОЯЩИМИ переносами строк: абзацы (1-2 предложения) разделяй пустой строкой. НЕ пиши последовательность из обратного слэша и n как текст — ставь реальный перенос строки.`;
}
