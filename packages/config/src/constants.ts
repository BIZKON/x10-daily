...
export const MODEL_COSTS: Record<string, { input: number; output: number }> = {
  ...
  //
  // ⚠️ ЭТО ОЦЕНКА, НЕ ПОДТВЕРЖДЁННЫЙ ТАРИФ — сверить строку модели в ЛК Timeweb
  // и поправить. Пока она здесь, $-ledger хотя бы видит расход на картинки, а не
  // считает их бесплатными.
  //
  // Как получена: живой замер usage одной генерации 1K (05.08.2026) —
  //   prompt_tokens 52, completion_tokens 1514 (из них image_tokens 1120,
  //   text_tokens 394). Канон `packages/voice/visual.md` оценивает 1K-картинку
  //   в ≈10-12 ₽ на тарифе Timeweb; берём 11 ₽ за изображение:
  //   11 ₽ / 1514 ток. = 0.00727 ₽/ток → 7266 ₽/М ÷ 71.73 ₽/$ ≈ $101/М output.
  //   Вход (52 токена промпта) в стоимости картинки пренебрежимо мал — ставим
  //   ту же ставку, что и у текстовых DeepSeek-моделей, чтобы не занижать.
  //
  // Проверка на разумность: 4 статьи/день × ~1514 ток. × $101/М ≈ $0.61/день
  // ≈ 1 300 ₽/мес — сходится с прикидкой канона (~1 500 ₽/мес).
  "gemini/gemini-3.1-flash-image-preview": { input: 0.265, output: 101.0 },
  // NB Pro (hero-исключение канона, вдвое дороже) — на случай переключения.
  "gemini/gemini-3-pro-image-preview": { input: 0.265, output: 202.0 },
};
