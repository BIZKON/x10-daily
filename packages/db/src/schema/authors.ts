...
export const authors = pgTable(
  "authors",
  {
    ...
    /**
     * ДОЛЖНОСТЬ — подпись под именем в байлайне: «Основатель ProAgent AI»,
     * «Главный редактор», «Приглашённый эксперт».
     *
     * ⚠️ Это НЕ права доступа. Права живут в `users.role` и описаны картой
     * `@x10/config/permissions` (Владелец · Редактор · Автор · Наблюдатель).
     * Колонки называются одинаково по историческим причинам, и перепутать их
     * легко: в интерфейсе они намеренно разведены — здесь «Должность», там
     * «Роль в команде». Переименовывать колонку не стали: это API-контракт
     * (`authors.role` уходит в miniapp), а цена путаницы снимается словами.
     */
