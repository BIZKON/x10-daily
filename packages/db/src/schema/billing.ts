...
export const clientBalance = pgTable("client_balance", {
  ...
  lowThresholdRub: numeric("low_threshold_rub", { precision: 14, scale: 4 })
    ...
    .default("500"),
  /**
   * Останавливать ли конвейер при нуле. У клиента — true. У НАШЕЙ копии false:
   * мы платим шлюзу напрямую и сами себе не переводим, остаток тут уходит в
   * минус и должен уходить. На шаге 1 флаг ещё не читается — заведён заранее,
   * чтобы шаг 2 не остановил наш собственный завод в момент выкатки.
   */
  stopOnZero: boolean("stop_on_zero").notNull().default(true),
