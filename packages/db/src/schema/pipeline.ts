...
export const agentKind = pgEnum("agent_kind", [
  ...
  "rewrite",
  /**
   * Создание материала по запросу человека — ручной режим. Миграция 0026.
   *
   * Отдельный вид, а не `draft`: экран «Расходы» группирует по виду агента, и
   * «линия сделала» против «ваш сотрудник попросил» — разные разговоры с
   * клиентом, которому мы этот счёт выставляем.
   */
  "creation",
  /**
   * Обход сайта клиента для базы знаний. Миграция 0029.
   *
   * Отдельно от `creation` по той же причине: разовая настройка базы и
   * ежедневная работа сотрудника — разные строки в счёте клиента.
   */
  "knowledge",
  /**
   * Разбор материала на слайды карусели. Миграция 0037.
   *
   * Отдельно от `visual` по той же причине: обложка и карусель — разные строки
   * в счёте клиента, и на экране «Расходы» их видно порознь.
   */
  "carousel",
  /**
   * Сборка контент-плана на месяц. Миграция 0032.
   *
   * Отдельно от `creation` по той же причине: разовая сборка плана и ежедневная
   * работа сотрудника — разные строки в счёте клиента.
   */
  "plan",
  "brevity",
  "audio",
