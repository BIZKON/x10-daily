...
async function main() {
  ...
  const tallies: Record<string, Tally> = {
    draft: t(), numbers: t(), tov: t(), brevity: t(), factcheck: t(), hookgen: t(), social: t(), score: t(),
  };
  ...
  console.log(`draft: ${d.r ? "OK" : "FAIL"} ${d.ms}ms ${draft ? "tease=" + JSON.stringify((draft as { tease?: string }).tease)?.slice(0, 80) : tallies.draft.errors.at(-1)}`);

  if (draft) {
    const nb = await tryRun("numbers", tallies.numbers, () =>
      NumbersAgent.run({ text: serializeDraftForNumbers(draft as never), sources: sourcesBiz } as never, ctx),
    );
    console.log(`numbers: ${nb.r ? "OK" : "FAIL"} ${nb.ms}ms ${nb.r ? "" : tallies.numbers.errors.at(-1)}`);
    ...
    for (let i = 0; i < 5; i++) {
      ...
      console.log(`tov[${i + 1}/5]: ${tv.r ? "OK" : "FAIL"} ${tv.ms}ms ${out ? `changes=${out.changes?.length}` : tallies.tov.errors.at(-1)}`);
    }
    ...
    if (lastRevised) {
      ...
      console.log(`brevity: ${br.r ? "OK" : "FAIL"} ${br.ms}ms ${compressed ? `${compressed.length}ch` : tallies.brevity.errors.at(-1)}`);

      if (compressed) {
        const hg = await tryRun("hookgen", tallies.hookgen, () => HookGenAgent.run({ draft: compressed, channel: "tg-x10" } as never, ctx));
        console.log(`hookgen: ${hg.r ? "OK" : "FAIL"} ${hg.ms}ms ${hg.r ? "" : tallies.hookgen.errors.at(-1)}`);
        const so = await tryRun("social", tallies.social, () => SocialAmplifyAgent.run({ draft: compressed, channel: "tg-x10", authorName: null } as never, ctx));
        const post = (so.r as { output?: { post?: string } } | null)?.output?.post;
        console.log(`social: ${so.r ? "OK" : "FAIL"} ${so.ms}ms post[0:90]=${JSON.stringify(post?.slice(0, 90))}`);
        const sc = await tryRun("score", tallies.score, () => PreviewScoreAgent.run({ draft: compressed } as never, ctx));
        console.log(`score: ${sc.r ? "OK" : "FAIL"} ${sc.ms}ms ${sc.r ? "" : tallies.score.errors.at(-1)}`);
      }
    }
  }
  ...
  if (draftP) {
    const tvP = await tryRun("tov", tallies.tov, () => ToVAgent.run({ draft: draftP as never, authorName: null } as never, ctx));
    const revisedP = (tvP.r as { output?: { revised?: string } } | null)?.output?.revised;
    const brP = revisedP ? await tryRun("brevity", tallies.brevity, () => BrevityAgent.run({ revised: revisedP, template: "card-news" } as never, ctx)) : null;
    const compressedP = (brP?.r as { output?: { compressed?: string } } | null)?.output?.compressed ?? revisedP ?? evPol.context;
    for (let i = 0; i < 3; i++) {
      const fc = await tryRun("factcheck", tallies.factcheck, () =>
        FactCheckAgent.run({ draft: compressedP, sources: sourcesPol, topicContext: evPol.context } as never, ctx),
      );
      const out = (fc.r as { output?: { status?: string; verdict?: string; confidence?: string; haltReason?: string } } | null)?.output;
      console.log(`factcheck[${i + 1}/3]: ${fc.r ? "OK" : "FAIL"} ${fc.ms}ms ${out ? `status=${out.status} verdict=${out.verdict} conf=${out.confidence}` : tallies.factcheck.errors.at(-1)}`);
    }
  }

  console.log("\n=== ИТОГ (ok/всего) ===");
  for (const [k, v] of Object.entries(tallies)) {
    console.log(`${k.padEnd(10)} ${v.ok}/${v.ok + v.fail}${v.errors.length ? "  ⚠ " + v.errors[0] : ""}`);
  }
}
